    ...
    <div className="max-w-full mx-auto min-h-screen bg-[#064E3B] rounded-[24px] p-4 md:p-8 space-y-4 shadow-[0_40px_100px_rgba(0,0,0,0.4)] border border-white/5 fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 no-print">
        <div className="glass-card p-6 flex items-center gap-6 bg-white/5 border border-white/10 backdrop-blur-xl">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-blue-400 tracking-widest">Base Active</p>
        ...
        <div className="glass-card p-6 flex items-center gap-6 bg-white/5 border border-white/10 backdrop-blur-xl">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-[#F59E0B] tracking-widest">Membres VIP</p>
        ...
        <div className="glass-card p-6 flex items-center gap-6 bg-white/5 border border-white/10 backdrop-blur-xl">
          ...
          <div>
            <p className="text-[10px] font-black uppercase text-red-400 tracking-widest">Comptes à Risque</p>
      ...
      <div className="glass-card rounded-[24px] overflow-hidden border border-white/10 shadow-2xl bg-white/5 backdrop-blur-2xl">
        ...
        <div className="block lg:hidden divide-y divide-navy-50">
          {clientData.length > 0 ? clientData.map((c, i) => {
            ...
            return (
              <div key={i} className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {isRisk ? (
                    <span className="bg-red-50 text-red-500 px-2 py-1 rounded-full text-[9px] font-black uppercase border border-red-100 flex items-center gap-1"><AlertTriangle className="w-3 h-3"/>Risk</span>
                  ) : isVIP ? (
                    <span className="bg-gold/10 text-gold px-2 py-1 rounded-full text-[9px] font-black uppercase border border-gold/20 flex items-center gap-1"><Award className="w-3 h-3"/>VIP</span>
                  ) : (
                    <span className="text-blue-gray border border-blue-gray/10 px-2 py-1 rounded-full text-[9px] font-black uppercase">Standard</span>
                  )}
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-navy-50 rounded-lg p-2 text-center">
                    <p className="text-[8px] text-blue-gray uppercase font-black">Transactions</p>
                    <p className="text-xs font-black text-charcoal">{c.transactions}</p>
                  </div>
                  <div className="bg-navy-50 rounded-lg p-2 text-center">
                    <p className="text-[8px] text-blue-gray uppercase font-black">Total</p>
                    <p className="text-xs font-black text-navy-900">{store.formatCurrency(c.totalSpent)}</p>
                  </div>
                  <div className="bg-navy-50 rounded-lg p-2 text-center">
                    <p className="text-[8px] text-blue-gray uppercase font-black">Debt</p>
            ...
            <div className="p-12 text-center text-blue-gray font-black uppercase tracking-widest text-sm">No Client Records Found</div>
          )}
        </div>
        ...
        <div className="hidden lg:block overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-white/5">
              {clientData.map((c, i) => {
                 ...
                 return (
                    <tr key={i} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="p-6 text-center text-[10px] font-black uppercase tracking-widest flex items-center justify-center">
                         {isRisk ? (
              ...
              {clientData.length === 0 && <tr><td colSpan="5" className="p-20 text-center text-white/40 font-black uppercase tracking-widest">No Client Records Found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
