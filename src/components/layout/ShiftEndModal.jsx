      ...
      <div className="bg-white max-w-2xl w-full rounded-[48px] shadow-[0_32px_80px_rgba(0,0,0,0.3)] overflow-hidden border border-navy-50 scale-in">
        <div className="p-10 border-b border-navy-50 flex items-center justify-between">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h3 className="text-2xl font-black text-navy-950 uppercase tracking-tighter">{L('Shift End', 'Fin de Poste')}</h3>
                <p className="text-xs font-bold text-blue-gray uppercase tracking-widest italic">{store.currentOperator} ({L('Active Session', 'Session Active')})</p>
             </div>
          </div>
        ...
        <div className="p-10 space-y-8 max-h-[60vh] overflow-y-auto scrollbar-thin">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="bg-navy-50 p-6 rounded-3xl border border-navy-100">
                <p className="text-xs font-black uppercase text-blue-gray mb-1">{L('Shift Start', 'Début du Poste')}</p>
                <p className="text-lg font-black text-navy-950">{new Date(store.shiftStart).toLocaleString(lang === 'fr' ? 'fr-FR' : 'en-US')}</p>
             </div>
             <div className="bg-navy-900 p-6 rounded-3xl border border-navy-800">
                <label className="text-xs font-black uppercase text-white/40 mb-2 block tracking-widest">{L('Departure Time (Editable)', 'Heure de Départ (Modifiable)')}</label>
                <input 
                  type="datetime-local" 
                  value={endTime}
          ...
          <div className="space-y-4">
             <div className="flex items-center justify-between px-2">
                <p className="text-xs font-black uppercase tracking-widest text-blue-gray flex items-center gap-2">
                  <ShoppingCart className="w-4 h-4" /> {L('Your Transactions', 'Vos Transactions')} ({shiftData.sales.length})
                </p>
                <p className="text-xs font-black uppercase tracking-widest text-navy-brand italic">{L('Strictly yours', 'Strictement à vous')}</p>
             </div>
             <div className="divide-y divide-navy-50 border border-navy-50 rounded-[32px] overflow-hidden bg-white shadow-sm">
                ...
                {shiftData.sales.length === 0 && (
                  <div className="p-10 text-center text-blue-gray text-xs font-black uppercase tracking-widest opacity-40 italic">
                    {L('No transactions performed during this shift.', 'Aucune transaction effectuée durant ce poste.')}
                  </div>
                )}
             </div>
          </div>
            ...
            <div className="space-y-3">
               <div className="flex items-center justify-between px-2">
                  <p className="text-xs font-black uppercase tracking-widest text-amber-500 flex items-center gap-2">
                    <Star className="w-4 h-4" /> {L('Tips Received', 'Pourboires Reçus')} ({shiftData.tips.length})
                  </p>
           ...
           {!isCounted ? (
              <div className="w-full flex flex-col items-center gap-4">
                 <div className="w-full max-w-md space-y-2">
                    ...
                    <input 
                       ...
                       placeholder={L('Enter counted amount...', 'Saisissez le montant compté...')}
                 ...
                 <button 
                   onClick={() => {
                     if (blindCount === '') {
                        store.showAlert(L("Please enter the counted amount.", "Veuillez saisir le montant compté."), "warning");
                        return;
                     }
                     setIsCounted(true);
                   }}
                 ...
                 >
                   {L('Validate Count', 'Valider le Comptage')} <CheckCircle2 className="w-5 h-5" />
                 </button>
              </div>
           ) : (
              <div className="w-full space-y-6">
                 ...
                 <div className="flex justify-end">
                    ...
                    >
                      {L('Confirm & Close Shift', 'Confirmer & Fermer le Poste')} <CheckCircle2 className="w-5 h-5" />
                    </button>
                 </div>
              </div>
           )}
